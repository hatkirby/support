...
std::string sentence::generate() const
{
  ...
  verbly::token form = actions[
    std::uniform_int_distribution<int>(0, actions.size()-1)(rng_)];
  ...
  std::string compiled = form.compile();
  std::list<std::string> words =
    verbly::split<std::list<std::string>>(compiled, " ");
  ...
  for (std::string& word : words)
  {
    ...
    {
      cased << verbly::token::capitalize(verbly::token::casing::all_caps, word);
    } else {
      cased << word;
    }
  }

  return cased.compile();
}
  ...
  for (const verbly::part& part : parts)
  {
    switch (part.getType())
    {
      ...
      case verbly::part_type::verb:
      {
        ...
        } else if (it.hasSynrestr("gerund_phrase"))
        {
          utter << verbly::token(verb, verbly::inflection::past_participle);
        } else {
          utter << verb;
...
void sentence::visit(verbly::token& it) const
{
  switch (it.getType())
  {
    ...
    {
      if (it.hasSynrestr("infinitive_phrase") || it.hasSynrestr("gerund_phrase"))
      {
